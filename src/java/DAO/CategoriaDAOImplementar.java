...
public class CategoriaDAOImplementar implements CategoriaDAO{
    ...
    public Categoria editarCat(int id_cat_edit){
       ...
       Categoria categoria = new Categoria();
       StringBuilder miSQL = new StringBuilder();
       miSQL.append ("SELECT * FROM TB_CATEGORIA WERE ID_CATEGORIA =").append(id_cat_edit);
       try{ 
           ResultSet resultadoSQL = this.conn.consultaSQL(miSQL.toString());
           while(resultadoSQL.next()){
               categoria.setId_categoria(resultadoSQL.getInt("id_categoria"));
    ...
    public boolean GuardarCat(Categoria categoria){
        ...
        try{
            if(categoria.getId_categoria() == 0){
                ...
                StringBuilder miSQL = new StringBuilder();
                miSQL.append("UPDAPE tb_categoria SET id_categoria =").append(categoria.getId_categoria());
